        ...
        implements LongStream
{
    ...
    {
        // extract the number of fixed bits
        int fb = LongDecode.decodeBitWidth((firstByte >>> 1) & 0x1F); // 0b1_1111
        ...
        int baseWidth = (thirdByte >>> 5) & 0x07; // 0b0111
        // base width is one off
        baseWidth += 1;

        // extract patch width
        int patchWidth = LongDecode.decodeBitWidth(thirdByte & 0x1F); // 0b1_1111

        // read fourth byte and extract patch gap width
        int fourthByte = input.read();
        int patchGapWidth = (fourthByte >>> 5) & 0x07; // 0b0111
        // patch gap width is one off
        patchGapWidth += 1;

        // extract the length of the patch list
        int patchListLength = fourthByte & 0x1F; // 0b1_1111
    ...
    {
        // extract the number of fixed bits
        int fixedBits = LongDecode.decodeBitWidth((firstByte >>> 1) & 0x1F); // 0b1_1111
            ...
            throws IOException
    {
        ...
        int size = (firstByte >>> 3) & 0x07; // 0b0111
        // #bytes are one off
        size += 1;

...
public class TestCompressionStorages {
  ...
  private void storageCompressionTest(String storeType, Class<? extends CompressionCodec> codec) throws IOException {
    ...
    Appender appender = ((FileTablespace) TableSpaceManager.getByName("local").get()).getAppender(meta, schema, tablePath);
    ...
    Scanner scanner = TableSpaceManager.getByName("local").get().getScanner(meta, schema, tablets[0], schema);

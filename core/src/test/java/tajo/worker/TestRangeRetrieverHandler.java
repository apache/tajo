...
public class TestRangeRetrieverHandler {
  ...
  public void testGet() throws Exception {
    ...
    TableDesc employee = new TableDescImpl("employee", employeeMeta,
    ...
    BSTIndex.BSTIndexReader reader = bst.getIndexReader(
        new Path(workDir.getAbsolutePath(), "out/index/data.idx"), keySchema, comp);
    reader.open();
    FileScanner scanner = (FileScanner) sm.getLocalScanner(
        new Path(workDir.getAbsolutePath(), "out"), "data");
    ...
    RangeRetrieverHandler handler = new RangeRetrieverHandler(
        new File(workDir, "out"), keySchema, comp);
    ...
    long fileLength = new File(workDir, "out/data/data").length();
  ...
  public void testGetFromDescendingOrder() throws Exception {
    ...
    File workDir = TajoTestingUtility.getTestDir("testGetFromDescendingOrder");
    TaskAttemptContext
        ctx = new TaskAttemptContext(conf, TUtil.newQueryUnitAttemptId(),
        new Fragment[] {frags[0]}, workDir);
    ...
    File dataFile = new File(workDir.getAbsolutePath(), "out");

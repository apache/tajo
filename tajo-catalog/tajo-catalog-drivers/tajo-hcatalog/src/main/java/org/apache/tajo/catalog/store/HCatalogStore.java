    ...
    try {
      ...
      if (properties != null) {
        ...
        if (storeType.equals(CatalogProtos.StoreType.CSV)) {
          options.set(StorageConstants.TEXTFILE_DELIMITER, StringEscapeUtils.escapeJava(fieldDelimiter));
          options.set(StorageConstants.TEXTFILE_NULL, StringEscapeUtils.escapeJava(nullFormat));
  ...
  public final void createTable(final CatalogProtos.TableDescProto tableDescProto) throws CatalogException {
    ...
    try {
      ...
      if (tableDesc.getMeta().getStoreType().equals(CatalogProtos.StoreType.RCFILE)) {
        ...
        String fieldDelimiter = tableDesc.getMeta().getOption(StorageConstants.TEXTFILE_DELIMITER,
        ...
        sd.getSerdeInfo().putToParameters(serdeConstants.FIELD_DELIM,
            StringEscapeUtils.unescapeJava(fieldDelimiter));
        table.getParameters().remove(StorageConstants.TEXTFILE_DELIMITER);

        if (tableDesc.getMeta().containsOption(StorageConstants.TEXTFILE_NULL)) {
          table.putToParameters(serdeConstants.SERIALIZATION_NULL_FORMAT,
              StringEscapeUtils.unescapeJava(tableDesc.getMeta().getOption(StorageConstants.TEXTFILE_NULL)));
          table.getParameters().remove(StorageConstants.TEXTFILE_NULL);
        }

...
public class TajoThriftServiceImpl implements TajoThriftService.Iface {
  ...
  public TajoClient getTajoClient(SessionIdProto sessionId) throws TServiceException {
    ...
    synchronized (tajoClientMap) {
      ...
      if (tajoClientHolder == null) {
        ...
        try {
          TajoClient tajoClient = new TajoClientImpl(ServiceTrackerFactory.get(tajoConf));
  ...
  public TGetQueryStatusResponse submitQuery(String sessionIdStr, String query, boolean isJson) throws TException {
    ...
    try {
        QueryClient tajoClient = getTajoClient(sessionId);
      ...
      if (clientResponse.getIsForwarded()) {
        ...
        ResultSet resultSet = TajoClientUtil.createResultSet(tajoClient, clientResponse,ThriftServerConstants.DEFAULT_FETCH_SIZE);
    ...
    if (rsHolder == null) {
      ...
      try {
        ...
        if (rsHolder.getTableDesc() != null)  {
          ...
          schema = rsHolder.getResultSet().getSchema();
          queryResult.setSchema(TajoThriftUtil.convertSchema(schema));
        }
  ...
  public TServerResponse createSession(String userId, String defaultDatabase) throws TException {
    try {
      ...
      if (tajoClientMap.size() >= maxSession) {
        ...
        TajoClient tajoClient = new TajoClientImpl(ServiceTrackerFactory.get(tajoConf), dbName);
  ...
  class QuerySubmitTask extends Thread {
    ...
    public void run() {
      ...
      try {
        FetchResultSet rs = null;
        if (queryFinished && TajoProtos.QueryState.QUERY_SUCCEEDED.name().equals(lastResponse.getState())) {
          if (lastResponse.isHasResult()) {
            rs = (FetchResultSet)getTajoClient(queryProgressInfo.sessionId).getQueryResult(queryProgressInfo.queryId);
          } else {
            rs = (FetchResultSet)createNullResultSet(getTajoClient(queryProgressInfo.sessionId), queryProgressInfo.queryId);
          }
        } else {
...
//        resultSetHolder.setTableDesc(rs.getTableDesc());
  ...
  public ResultSet createNullResultSet(TajoClient client, QueryId queryId) throws IOException {
    return new FetchResultSet(client, null, queryId, ThriftServerConstants.DEFAULT_FETCH_SIZE);
  }

...
import org.apache.tajo.thrift.CallQueue.Call;
...
/**
 * A bounded thread pool server customized for Tajo.
 */
...
public class TBoundedThreadPoolServer extends TServer {
  ...
  public static final String MAX_WORKER_THREADS_CONF_KEY =
      "tajo.thrift.maxWorkerThreads";

  public static final int DEFAULT_MAX_WORKER_THREADS = 100;
   ...
   * tajo.thrift.maxQueuedRequests threads.
   */
  public static final String MAX_QUEUED_REQUESTS_CONF_KEY =
      "tajo.thrift.maxQueuedRequests";

  public static final int DEFAULT_MAX_QUEUED_REQUESTS = 100;
  ...
  public static final String THREAD_KEEP_ALIVE_TIME_SEC_CONF_KEY =
      "tajo.thrift.threadKeepAliveTimeSec";
  ...
  public TBoundedThreadPoolServer(Args options) {
    ...
    if (options.maxQueuedRequests > 0) {
      this.callQueue = new CallQueue(
          new LinkedBlockingQueue<Call>(options.maxQueuedRequests));
    } else {
      this.callQueue = new CallQueue(new SynchronousQueue<Call>());
    }
  ...
  public void serve() {
    ...
    while (!stopped && !Thread.interrupted()) {
      ...
      try {
        ...
        ServerSocket serverSocket = ((TServerSocket)serverTransport_).getServerSocket();
        if (!stopped) {
          ...
          retry++;
          if (retry > 10) {
            LOG.fatal("Transport error when accepting message after retrying 10 times", ttx);
            System.exit(1);
          }
          continue;
        } else {
  ...
  /**
   * Loop until {@link java.util.concurrent.ExecutorService#awaitTermination} finally does return
  ...
  private void shutdownServer() {
    ...
    sleepWithoutInterrupt(TIME_TO_WAIT_AFTER_SHUTDOWN_MS);
   ...
   */
  private static void sleepWithoutInterrupt(final long msToWait) {
    long timeMillis = System.currentTimeMillis();

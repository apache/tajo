...
public enum SessionVars implements ConfigKey {
  ...
  //
  // * LIMIT   - We use the suffix 'LIMIT' if the variable is limit. So, if some value is greater or less than
  ...
  BROADCAST_NON_CROSS_JOIN_LIMIT(ConfVars.$DIST_QUERY_BROADCAST_NON_CROSS_JOIN_LIMIT,
      "restriction for the total size of broadcasted table for non-cross join (kb)", DEFAULT, Long.class,
      Validators.min("0")),
  BROADCAST_CROSS_JOIN_LIMIT(ConfVars.$DIST_QUERY_BROADCAST_CROSS_JOIN_LIMIT,
  ...
  INDEX_SELECTIVITY_LIMIT(ConfVars.$INDEX_SELECTIVITY_LIMIT, "the selectivity limit for index scan",
      DEFAULT),
  ...
  TEST_TIM_SORT_LIMIT_FOR_RADIX_SORT(ConfVars.$TEST_TIM_SORT_LIMIT_FOR_RADIX_SORT, "(test only) Tim sort limit for radix sort", TEST_VAR)
  ;

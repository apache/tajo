...
package org.apache.tajo.plan.function.python.executor;
...
import org.apache.tajo.plan.expr.StreamingUtil;
...
public class PythonScriptExecutor {
  ...
  public PythonScriptExecutor(OverridableConf queryContext) {
    this.queryContext = queryContext;
  }

  public void init(FunctionDesc functionDesc) throws IOException {
    FunctionSignature functionSignature = functionDesc.getSignature();
    ...
    startUdfController(functionDesc.getInvocation().getPython());
    createInputHandlers();
    setStreams();
    startThreads(functionDesc);
  }

  private StreamingCommand startUdfController(PythonInvocationDesc invocationDesc) throws IOException {
    StreamingCommand sc = new StreamingCommand(buildCommand(invocationDesc));
...
//  }
  ...
  private void startThreads(FunctionDesc functionDesc) {
    stdinThread = new ProcessInputThread();
    stdinThread.start();

    stdoutThread = new ProcessOutputThread(functionDesc.getFunctionName());
    stdoutThread.start();
  ...
  public Datum getOutput(Tuple input) {
    if (outputQueue == null) {
...
//    LOG.info("out: " + out);
  ...
  class ProcessOutputThread extends Thread {
    String funcName;
    ProcessOutputThread(String funcName) {

...
public class Task2 {
  ...
  public Task2(QueryUnitAttemptId taskId,
    ...
    this.context = new TaskAttemptContext2(conf, taskId,
...
//          part.setFileName(dataServerURL + "/?qid=" + getId().toString());
  ...
  private class FetchRunner implements Runnable {
    private final TaskAttemptContext2 ctx;
    private final Fetcher fetcher;

    public FetchRunner(TaskAttemptContext2 ctx, Fetcher fetcher) {
      this.ctx = ctx;
  ...
  private List<Fetcher> getFetchRunners(TaskAttemptContext2 ctx,

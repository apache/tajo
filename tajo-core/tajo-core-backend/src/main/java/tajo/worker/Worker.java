...
public class Worker extends Thread implements AsyncWorkerProtocol {
  ...
  private void prepareServing() throws IOException {
    ...
    this.workerId = BuilderUtils.newNodeId(isa.getAddress().getHostAddress(), isa.getPort());
  ...
  private void participateCluster() throws Exception, InterruptedException,
    ...
    do {
      masterAddrBytes = masterAddrTracker.blockUntilAvailable(500);
            ...
            try {
              ...
              if (!stopped) {
                if (callFuture == null) {
                  ...
                  //master.getTask(null, ((NodeIdPBImpl)workerId).getProto(), callFuture);
                }
                try {

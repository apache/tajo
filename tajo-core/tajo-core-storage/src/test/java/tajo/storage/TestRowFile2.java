...
public class TestRowFile2 {
	private TajoConf conf;
	private static String TEST_PATH = "target/test-data/TestRowFile2";
  ...
  public void testRowFile() throws IOException {
    ...
    sm.initTableBase(meta, "testRowFile");
    Appender appender = sm.getAppender(meta, "testRowFile", "file1");
    int tupleNum = 10000;
    VTuple vTuple;
    ...
    FileStatus status = sm.listTableFiles("testRowFile")[0];
    long fileLen = status.getLen();
    long randomNum = (long) (Math.random() * fileLen) + 1;
    
    Fragment[] tablets = new Fragment[2];
    tablets[0] = new Fragment("testRowFile", status.getPath(), meta, 0, randomNum, null);
    tablets[1] = new Fragment("testRowFile", status.getPath(), meta, randomNum, (fileLen - randomNum), null);
  ...
  public void testVariousTypes() throws IOException {
    ...
    TableMeta meta = TCatUtil.newTableMeta(schema, StoreType.ROWFILE, options);

    sm.initTableBase(meta, "row");
    Appender appender = sm.getAppender(meta, "row", "table.dat");
    ...
    Scanner scanner = sm.getScanner("row", "table.dat");

...
//    conf.setBoolVar(ConfVars.$TEST_FILTER_PUSHDOWN_ENABLED, true);
...
//    sortKeys = new SortSpec[1];
//    sortKeys[0] = new SortSpec(idxSchema.getColumn("managerid"), true, false);
//    TupleComparator comp = new BaseTupleComparator(idxSchema, sortKeys);
//
//    this.writer = new BSTIndex(conf).getIndexWriter(idxPath,
//        BSTIndex.TWO_LEVEL_INDEX, this.idxSchema, comp);
...
//    optimizer = new LogicalOptimizer(conf, catalog, new AccessPathRewriterContext(true));
//  }
//
//  @After
...
//
//      SimplePredicate[] predicates = new SimplePredicate[1];
//      predicates[0] = new SimplePredicate(sortKeys[0], DatumFactory.createInt4(rndKey));
//
//      return new BSTIndexScanExec(ctx, sm, (IndexScanNode) scanNode, fragments.get(0), idxPath, idxSchema, predicates);
//
//    }
//  }
//}